...
class ctwitter_stream
{
    ...
    private function process_tweet(array $_data)
    {
        ...
        //print_r($_data['text']);
        //$arrayText = print_r($_data['text'], true);
        global $arrayText;
        //foreach ($_data as $value) {
            array_push($arrayText, $_data['text']);
        //}
        //echo "DISTRACTION";
        //print_r($arrayText);
        $response['arraytext'] = $arrayText;
        $fp = fopen('results.json', 'w');
        fwrite($fp, json_encode($response));
        fclose($fp);
        //echo $arrayText;
            /*if (strpos($arrayText, 'Hillary') !== false) {
                global $countHillary;
                $countHillary++;
            }*/ 
        
        //echo substr_count($arrayText, 'Bernie');
        //echo "hillarys count: " . $countHillary;
